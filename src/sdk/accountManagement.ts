...
export type OverdraftAccountResponse =
  components["schemas"]["OverdraftAccountResponse"];
export type OverdraftAccountStatus =
  components["schemas"]["OverdraftAccountStatus"];
export type EONWalletAccountUpgradeRequest =
  components["schemas"]["EONWalletAccountUpgradeRequest"];
export type EONWalletAccountUpgradeResponse =
  components["schemas"]["EONWalletAccountUpgradeResponse"];
...
export async function submitAccountApplication({
  ...
  const res = await fetchImpl(
    `${baseUrl}/dao/v1/savings/accounts/applications/submit`,
    {
      method: "POST",
      headers: {
...
export async function createOverdraftAccount({
  ...
  accessToken: string;
  partnerId: string;
  body: components["schemas"]["DAOSubmitRequest"];
  fetchImpl?: Fetch;
  baseUrl?: string;
...
export async function getOverdraftAccountStatus({
  ...
  requireParam(partnerId, "partnerId");
  const res = await fetchImpl(
    `${baseUrl}/partners/sb/partners/v1/accounts/overdraft`,
    {
      method: "GET",
      headers: {
        accept: "application/json",
...
export async function upgradeEONWalletAccount({
  ...
  requireParam(body, "body");
  const res = await fetchImpl(
    `${baseUrl}/partners/eon/wallet/v1/accounts/upgrade`,
    {
      method: "POST",
      headers: {
        accept: "application/json",
